        ...
        while (current != NULL) {
            if (!details) {
                ...
                if (ch == '\1') {
                    ...
                    printf("\033[32m"); // colore rosso
                } else if (ch == '\3') {
                    printf("\033[39m"); // colore di default
                } else {
                    printf("%c", ch);
                }
...
int fscore(char c1, char c2) {
    ...
    if (c1 == c2) {
        ...
        (c1 >= 'a' && c1 <= 'z' || c1 >= 'A' && c1 <= 'Z' || c1 == '_') &&
        (c2 >= 'a' && c2 <= 'z' || c1 >= 'A' && c1 <= 'Z' || c2 == '_')
    ) {
        ...
        // entrambi sono simboli matematici
        return 3;
    }
...
void allinea(string s1, string s2) {
        ...
        for (int j = 1; j < len2; j++) {
            ...
            if (match) { // match
                ...
                for (char ws : whitespaces) {
                    if (str1[i] == ws && str2[j - 1]) {
                        tolerant_M[i][j] = tolerant_M[i][j - 1];
                        substr_P[i][j] = 1;
                        break;
                    } else if (str1[i - 1] == ws && str2[j]) {
    ...
    while (substr_P[i][j] != -1) {
        switch (substr_P[i][j]) {
            case 0:
                list_insert_front(longest_substr, '\3');
